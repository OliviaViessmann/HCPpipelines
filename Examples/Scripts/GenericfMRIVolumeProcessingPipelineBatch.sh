#!/bin/bash 
# derivative of GenericfMRIVolumeProcessingPipelineBatch.sh to run processing script that warps fmri data into native T1w space and not MNI
...
## *****************OV adding things for orientation analysis *************************
#
#
#OV changed to adaped script
    ${queuing_command} ${HCPPIPEDIR}/fMRIVolume/GenericfMRIVolumeProcessingPipeline2Native.sh \
      --path=$StudyFolder \
      --subject=$Subject \
      --fmriname=$fMRIName \
