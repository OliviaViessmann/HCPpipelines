...
########### OV transform surfaces to MNI volume space
for Hemisphere in lh rh; do

	#native Mesh Processing
	#Convert and volumetrically register surfaces to MNI volume space, convert to mgz format by the end
	for Surface in midgray subwhite ; do
		for depth in `seq -f %02.0f 0 10`; do
			mris_convert "$FreeSurferFolder"/surf/${Hemisphere}."$Surface".${depth} "$T1wFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii
			${CARET7DIR}/wb_command -surface-apply-affine "$T1wFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii "$FreeSurferFolder"/mri/c_ras.mat "$T1wFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii
			${CARET7DIR}/wb_command -surface-apply-warpfield "$T1wFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii "$InverseAtlasTransform".nii.gz "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii -fnirt "$AtlasTransform".nii.gz
			mris_convert "$T1wFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii "$T1wFolder"/"$NativeFolder"/${Hemisphere}."$Surface".${depth}.native.surf
			mris_convert "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject".${Hemisphere}."$Surface".${depth}.native.surf.gii "$AtlasSpaceFolder"/"$NativeFolder"/${Hemisphere}."$Surface".${depth}.native.surf  
		done
	done
done
